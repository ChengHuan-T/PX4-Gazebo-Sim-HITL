...
This code script subscribes the IMU, Mag, Baro and GPS topic, and publishes the motor_speed topic.
"""
...
# Add system site-packages for gz bindings
...
# --- defaults ---
SERIAL_PORT = "/dev/ttyACM*"   # Modify with your serial port
BAUD_RATE = 921600
WORLD_NAME = "hitl"
MODEL_NAME = "x500"
...
class HITLBridge:
    def __init__(self, serial_port, baud_rate, world_name, model_name):
        ...
        self.mag_last_time = time.time()
        self.baro_last_time = time.time()
        self.gps_last_time = time.time()

        self.motor_count = 0
        self.motor_last_time = time.time()
    ...
    def print_rate(self, name, count, last_time, window):
        if count % window == 0:
            now = time.time()
            rate = window / (now - last_time)
    ...
    def _setup_subscribers(self):
        topics = {
            "imu_sensor/imu": (imu_pb2.IMU, self._on_imu),              # IMU
            "magnetometer_sensor/magnetometer": (                       # Mag
                magnetometer_pb2.Magnetometer,
                self._on_mag,
            ),
            "air_pressure_sensor/air_pressure": (                       # Baro
                fluid_pressure_pb2.FluidPressure,
                self._on_baro,
            ),
            "navsat_sensor/navsat": (navsat_pb2.NavSat, self._on_gps),  # GPS
        }
    ...
    def _setup_publisher(self):
        topic = f"/{self.model}/command/motor_speed"                    # Published topic name
    ...
    def _on_imu(self, msg):
        with self.lock:
            ...
            self.sim_time_us = int(time.monotonic() * 1e6)
            self.imu_data = msg
            self.new_imu = True
        self.imu_count += 1
        self.imu_last_time = self.print_rate(
            "IMU", self.imu_count, self.imu_last_time, 250           # IMU 250 Hz
        )
    ...
    def _on_mag(self, msg):
        with self.lock:
            ...
            self.mag_data = msg
        self.mag_count += 1
        self.mag_last_time = self.print_rate(
            "MAG", self.mag_count, self.mag_last_time, 100           # MAG 100 Hz
        )
    ...
    def _on_baro(self, msg):
        with self.lock:
            ...
            self.baro_data = msg
        self.baro_count += 1
        self.baro_last_time = self.print_rate(
            "BARO", self.baro_count, self.baro_last_time, 50         # BARO 50 Hz  
        )
    ...
    def _on_gps(self, msg):
        with self.lock:
            ...
            self.new_gps = True
        self.gps_count += 1
        self.gps_last_time = self.print_rate(
            "GPS", self.gps_count, self.gps_last_time, 30            # GPS 30 Hz 
        )
    ...
    def _send_hil_sensor(self):
        ...
        if not hasattr(self, '_dumped'):
            ...
            print(f"  accel:    ({ax:.4f}, {ay:.4f}, {az:.4f})  [expect ~(0, 0, 0)]")
    ...
    def _handle_actuator_controls(self, msg):
        ...
        for i in range(4):
            ...
            u = max(0.0, msg.controls[i])
            speed = math.sqrt(u) * MAX_ROTOR_VELOCITY
        ...
        while self.running:
            ...
            while True:
                ...
                if mtype == "HIL_ACTUATOR_CONTROLS":
                    ...
                    # if self.motor_count % 100 == 0:
                    #     now = time.time()
                    #     rate = 100 / (now - self.motor_last_time)
                    #     print(f"[MOTOR PUB] {rate:.1f} Hz")
...
def main():
    parser = argparse.ArgumentParser(
        description="PX4 HITL Bridge for Gazebo Sim"
    )
